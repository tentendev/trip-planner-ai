...
const LoadingOverlay: React.FC<LoadingOverlayProps> = ({ language, partialMarkdown, elapsedMs, onCancel }) => {
  ...
  const stopButton = onCancel ? (
    <button
      type="button"
      onClick={onCancel}
      className="inline-flex items-center gap-2 px-5 py-2 rounded-xl border-2 border-red-300 dark:border-red-500/40 bg-white dark:bg-slate-900 text-red-600 dark:text-red-400 font-semibold text-sm hover:bg-red-50 dark:hover:bg-red-500/10 hover:border-red-400 active:scale-[0.98] transition shadow-sm"
    >
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/80 backdrop-blur-md">
      {hasPreview ? (
        ...
        <div className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl rounded-3xl shadow-2xl p-5 md:p-7 max-w-lg md:max-w-2xl w-full mx-4 text-left border border-white/20 dark:border-slate-700/40 animate-in fade-in zoom-in-95 duration-200 flex flex-col max-h-[85vh]">
          ...
          <div className="flex items-center gap-4 mb-4 flex-none">
            ...
            <div className="relative w-14 h-14 flex-none" aria-hidden="true">
              <div className="absolute inset-0 rounded-full border-2 border-dashed border-slate-300/70 dark:border-slate-600/70 loader-ring" />
              <div className="loader-orbit absolute inset-0">
                <div className="absolute left-1/2 top-0 -translate-x-1/2 -translate-y-1/2 rounded-full bg-white dark:bg-slate-800 p-0.5 shadow-sm shadow-blue-200/80 dark:shadow-none ring-1 ring-slate-100 dark:ring-slate-700">
              ...
              <div className="loader-badge absolute inset-0 flex items-center justify-center">
                <div className="w-8 h-8 rounded-full bg-gradient-to-br from-blue-500 via-indigo-500 to-indigo-600 flex items-center justify-center shadow-md shadow-blue-500/30 ring-2 ring-white/60 dark:ring-slate-900/50">
            ...
            <div className="min-w-0 flex-1">
              <h2 className="text-lg md:text-xl font-bold text-slate-800 dark:text-slate-100 leading-tight truncate">
                {t.loading.title}
              </h2>
            ...
            <div className="text-right flex-none">
              <span className="block font-mono text-3xl md:text-4xl font-bold text-slate-800 dark:text-slate-100 tabular-nums leading-none tracking-tight">
                {clockLabel}
              </span>
            </div>
          </div>
          ...
          {/* Thin progress bar */}
          <div className="w-full h-1.5 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden mb-4 flex-none">
          ...
          <div className="flex flex-col min-h-0 flex-1">
            ...
            <div
              ...
              className="max-h-[40vh] overflow-y-auto rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50/70 dark:bg-slate-800/50 p-4 md:p-5"
              aria-live="polite"
            >
        ...
        <div className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl rounded-3xl shadow-2xl p-8 md:p-12 max-w-md mx-4 text-center border border-white/20 dark:border-slate-700/40">
          ...
          <div className="relative w-40 h-40 mx-auto mb-6" aria-hidden="true">
            ...
            <div className="absolute inset-2 rounded-full bg-gradient-to-tr from-blue-200/60 via-purple-100/50 to-pink-200/50 dark:from-blue-500/20 dark:via-purple-500/10 dark:to-pink-500/15 blur-2xl" />

            {/* Dashed outer ring drifting counter-clockwise */}
            <div className="loader-ring absolute inset-0">
              <div className="absolute inset-0 rounded-full border-2 border-dashed border-slate-300/70 dark:border-slate-600/70" />
            </div>

            {/* Orbit carrying the plane clockwise around the globe */}
            <div className="loader-orbit absolute inset-0">
              <div className="absolute left-1/2 top-0 -translate-x-1/2 -translate-y-1/2 rounded-full bg-white dark:bg-slate-800 p-1.5 shadow-md shadow-blue-200/80 dark:shadow-none ring-1 ring-slate-100 dark:ring-slate-700">
            ...
            <div className="loader-badge absolute inset-0 flex items-center justify-center">
              <div className="w-20 h-20 rounded-full bg-gradient-to-br from-blue-500 via-indigo-500 to-indigo-600 flex items-center justify-center shadow-lg shadow-blue-500/30 ring-4 ring-white/60 dark:ring-slate-900/50">
          ...
          {/* Rotating Tips */}
          <div className="bg-blue-50/80 dark:bg-blue-500/10 rounded-xl p-4 border border-blue-100 dark:border-blue-500/20">
            <p className="text-xs text-blue-400 uppercase tracking-wider font-bold mb-2">
              {t.loading.tip_label}
            </p>
            <p className="text-sm text-blue-700 dark:text-blue-300 leading-relaxed transition-all duration-500">
          ...
          <div className="mt-6 flex items-center justify-between gap-4">
            <p className="text-xs text-slate-400 dark:text-slate-500 font-mono text-left">
              {t.loading.estimated_time}
            </p>
            {stopButton}
          </div>
