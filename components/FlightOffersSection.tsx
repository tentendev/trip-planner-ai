...
const FlightOffersSection: React.FC<FlightOffersSectionProps> = ({
  ...
  return (
    <section className="no-print px-6 md:px-10 py-8 md:py-10 border-b border-slate-100 dark:border-slate-800 bg-gradient-to-b from-slate-50/50 to-white dark:from-slate-800/50 dark:to-slate-900">
      ...
      {priceInsights?.lowest && (
        <div className="mb-4 text-xs text-slate-500 dark:text-slate-400 flex items-center gap-2 flex-wrap">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        {flights.map((f) => {
          ...
          return (
            <article
              key={f.rank}
              className="group relative bg-white dark:bg-slate-800/60 rounded-2xl border border-slate-200/70 dark:border-slate-700/70 p-4 md:p-5 hover:border-slate-300 dark:hover:border-slate-600 transition-all"
            >
              {/* Badges */}
              ...
              <div className="flex items-center gap-2.5 mb-4">
                {f.airline_logo ? (
                  <img
                    src={f.airline_logo}
                    alt={f.airlines.join(' ')}
                    className="w-7 h-7 rounded-md object-contain bg-white dark:bg-slate-100 border border-slate-100 dark:border-slate-700"
                  />
                ) : (
                  <div className="w-7 h-7 rounded-md bg-slate-100 dark:bg-slate-700/60 flex items-center justify-center">
                    <Plane className="w-3.5 h-3.5 text-slate-500 dark:text-slate-400" />
                  </div>
                )}
              ...
              <div className="flex items-center gap-2 md:gap-3 mb-4">
                <div className="text-center">
                  <div className="text-2xl md:text-[26px] font-bold text-slate-900 dark:text-slate-100 leading-none font-mono">
                    {formatTime(first.from_time)}
                  </div>
                  <div className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 mt-1 tracking-wider">
                    {first.from_code}
                  </div>
                ...
                <div className="flex-1 flex flex-col items-center px-2 min-w-0">
                  ...
                  <div className="relative w-full flex items-center">
                    ...
                    <Plane className="w-3 h-3 text-slate-400 dark:text-slate-500 mx-1 rotate-90" />
                    <div className="flex-1 h-px bg-slate-200 dark:bg-slate-700" />
                  </div>
                  <div className="text-[10px] mt-1">
                    {f.stops === 0 ? (
                      <span className="text-emerald-600 dark:text-emerald-400 font-semibold">{t.direct}</span>
                    ) : (
              ...
              {f.layovers && f.layovers.length > 0 && (
                <div className="flex flex-wrap gap-1.5 mb-4 text-[10px]">
                  {f.layovers.map((l, i) => (
                    <span
                      key={i}
                      className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-300 border border-amber-100 dark:border-amber-500/20"
                    >
              ...
              <div className="flex items-end justify-between pt-3 border-t border-slate-100 dark:border-slate-800">
                <div>
                  <div className="text-[11px] text-slate-500 dark:text-slate-400 font-medium">{t.totalPrice}</div>
